...
![Screenshot Gif](resources/screen_shot.gif)

# Interactive Network Graph on Stencil+D3
...
Data is provided from private source ([BuscoExtra](https://buscoextra.es)) where the identifiers and sensitive data that may lead to individual identification has been removed or changed to randomized values.
...
```
Where `appearances` in Nodes represents how many times that node appears in Links. Also, `interactions` in Links is the number of times that relation between `source` and `target` has existed. You may find raw datasets [here](src/assets/).
