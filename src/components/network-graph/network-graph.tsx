...
export class NetworkGraph implements ComponentInterface {
  ...
  componentDidLoad() {
    ...
    this.simulation = forceSimulation(this.nodes)
        ...
        .force("charge", forceManyBody().strength(-200).distanceMax(50000))
  ...
  drawNode(node: NetworkNode){
      ...
      if(node.type == NetworkNodeType.Worker){
        radius = this.nodeRadiusScale(node.appearances) / 2;
        this.context.fillStyle = "#1abc9c";
      } else {
        this.context.fillStyle = "#2980b9";
        radius = this.nodeRadiusScale(10);
      }
      ...
      this.context.strokeStyle = "solid";
